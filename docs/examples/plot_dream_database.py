...
dream_meta_cache = os.path.join(icemodels.optical_constants_cache_dir, 'dream_meta_table.ecsv')
dream_demo_files = glob.glob(
    os.path.join(icemodels.optical_constants_cache_dir, 'dream_H2O___CO2*')
)

can_run_full_demo = (
    not docs_offline_mode
    or (os.path.exists(dream_meta_cache) and len(dream_demo_files) >= 2)
)

if not can_run_full_demo:
    print(
        "Skipping DREAM gallery execution: ICEMODELS_DOCS_OFFLINE=1 and required "
        "DREAM cache files are unavailable."
    )
else:
