"""
4500K stellar SED + CT06 dust + pure CO ice at A_V = 0, 10, 50, 100.

X(CO) = N(CO)/N(H2) = 2.5e-4 (icemodels default, wrt H2 NOT total H).
N(H) = 2.21e21 * A_V cm^-2;  N(H2) = N(H) / 2.
For Av>0 only the fully-absorbed (extinction + ice) curve is shown,
normalized to its own peak within the plotted range.
"""
import glob
...
from icemodels import (plot_stellar_seds, atmo_model, absorbed_spectrum,
                       fluxes_in_filters, read_ocdb_file,
                       optical_constants_cache_dir)


co_table = read_ocdb_file(
...
for Av in [0, 10, 50, 100]:
    ...
    if N_CO > 0:
        fnu_iced = absorbed_spectrum(ice_column=N_CO * u.cm**-2,
    ...
    out = output_dir / f'{TEMP}KStellarSEDexample_Av{Av:03d}_pureCO.png'
    fig = axes[0].figure
    fig.canvas.draw()
